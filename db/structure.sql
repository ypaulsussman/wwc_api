...
--
-- Name: searchable_studies; Type: MATERIALIZED VIEW; Schema: public; Owner: -
--

CREATE MATERIALIZED VIEW public.searchable_studies AS
 SELECT to_tsvector(regexp_replace(studies.citation, '\(.*'::text, ''::text, 'g'::text)) AS author,
    to_tsvector(array_to_string(regexp_matches(studies.citation, '\)\. ([^\.]+)'::text, 'g'::text), ''::text)) AS title,
    to_tsvector(regexp_replace(studies.publication, '[\d]+'::text, ''::text, 'g'::text)) AS publication,
    regexp_replace(studies.publication_date, '[^\d]+'::text, ''::text, 'g'::text) AS pub_year,
    (studies.demographics_of_study_sample_english_language_learners)::integer AS percent_ell,
    (studies.demographics_of_study_sample_free_or_reduced_price_lunch)::integer AS percent_frpl,
    (studies.ethnicity_hispanic)::integer AS percent_hispanic,
    (studies.gender_male)::integer AS percent_male,
    studies.demographics_of_study_sample_international AS international,
    studies.multisite,
    studies.study_design,
    studies.citation,
    studies.study_page_url,
    studies.ericid,
    studies.id
   FROM public.studies
  WITH NO DATA;
...
CREATE INDEX searchable_studies_author_idx ON public.searchable_studies USING gin (author);


--
-- Name: searchable_studies_pub_year_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX searchable_studies_pub_year_idx ON public.searchable_studies USING btree (pub_year);


--
-- Name: searchable_studies_publication_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX searchable_studies_publication_idx ON public.searchable_studies USING gin (publication);


--
-- Name: searchable_studies_title_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX searchable_studies_title_idx ON public.searchable_studies USING gin (title);
